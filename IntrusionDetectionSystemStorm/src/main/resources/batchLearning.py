...
X = pd.read_csv("kddcup_train_10.csv", names=header_names)
y = X['attack_type']
...
pickle.dump(standard_scaler, open("standard_scaler", 'wb'))

for col in nominal_cols:
    le = LabelEncoderExt().fit(X_train[col])
    pickle.dump(le, open("label_encoder_" + col, 'wb'))
    X_train[col] = le.transform(X_train[col])
    X_test[col] = le.transform(X_test[col])

# label_encoder_y = LabelEncoderExt().fit(y_train)
# y_train = label_encoder_y.transform(y_train)
# pickle.dump(label_encoder_y, open("label_encoder_targets", 'wb'))
